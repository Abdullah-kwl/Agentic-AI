...
Streamlit chatbot that answers questions over PDFs uploaded in the sidebar. Same stack as
the `langchain-RAG` notebooks (bge-m3 embeddings, Chroma, Groq), wrapped in a LangGraph graph.
...
`GROQ_API_KEY` is read from `D:\Projects\Agentic-AI\.env`.
